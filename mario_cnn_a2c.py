"""
Super Mario Bros A2C Training with CNN-based Observations
"""
...
#from stable_baselines3.common.utils importmodel LinearSchedule
...
class TerminalLoggerCallback(BaseCallback):
    """
    Prints episode rewards/lengths as they finish (works with VecEnv).
    Also prints a heartbeat every `print_freq` timesteps.
    """
    def __init__(self, moving_avg=200000, print_freq=200_000, verbose=0):
        super().__init__(verbose)
        self.moving_avg = moving_avg
        self.print_freq = print_freq
        self.ep_rewards = deque(maxlen=moving_avg)
        self.ep_lengths = deque(maxlen=moving_avg)

    def _on_step(self) -> bool:
        # VecEnv exposes per-env infos
        infos = self.locals.get("infos", [])
        for info in infos:
            if "episode" in info:  # Monitor wrapper reports this when an episode ends
                r = info["episode"]["r"]
                l = info["episode"]["l"]
                ...
                print(
                    ...
                    f"ep_reward={r:.1f}  ep_len={l}  "
                    f"avg{self.moving_avg}={mean_r:.1f} (len {mean_l:.0f})"
                )

        # Heartbeat
        if self.model.num_timesteps % self.print_freq == 0:
            print(f"[{self.model.num_timesteps:,}] training…")

        return True

def find_wrapper(env, wrapper_cls):
    e = env
...
def make_mario_env(
    ...
    **kwargs,
    ...
    #vec_env_cls_kwargs={"start_method": "spawn", "wait_timeout": 60},  # If you still see hangs, try "spawn" instead of "forkserver"
    **kwargs,
    )
...
def create_experiment_folder(base_dir="results/a2c"):
...
if __name__ == "__main__":
    ...
    # Train on fixed stage 1-1 (same as eval and test)
    ...
    eval_env = make_mario_env(
        ...
        env_kwargs={"max_episode_steps": 9_999_999},  # ensure no 160-step cap
        monitor_dir=f"{log_dir}/eval",
    )

    # ---- persist & load VecNormalize stats instead of attribute copy ----
    norm_path = os.path.join(model_dir, "vecnormalize.pkl")
    
    # find the outer VecNormalize on the training env and save it
    train_norm = None
    e = train_env
    ...
    if train_norm is not None:
        train_norm.save(norm_path)
    
    # rebuild eval wrapper using the saved stats
    # (VecNormalize.load wraps the provided env)
    eval_env = VecNormalize.load(norm_path, eval_env)
    eval_env.training = False          # freeze stats
    eval_env.norm_reward = True        # keep reward normalization on
    # note: obs normalization is False in your setup, so obs_rms won’t matter

    checkpoint_callback = CheckpointCallback(
        save_freq=25000,  # Save every 500k steps (500k / 8 envs = 62.5k)
        save_path=f"{model_dir}/checkpoint",
        name_prefix="mario_A2C",
        save_replay_buffer=False,           # ensure not saving huge buffers
        save_vecnormalize=False,            # we save VecNormalize separately
        verbose=1,
    )

    stop_callback = StopTrainingOnNoModelImprovement(
        max_no_improvement_evals=20,  # Stop after 5M steps without improvement (10 * 500k = 5M)
    ...
    eval_callback = EvalCallback(
        ...
        eval_freq=25000,  # Evaluate every 500k steps (500k / 8 envs = 62.5k)
        n_eval_episodes=5,  # Deterministic env → only need 1 eval
        deterministic=False,
        render=False,
    ...
    term_logger = TerminalLoggerCallback(moving_avg=200000, print_freq=200_000)
    callbacks = [checkpoint_callback, eval_callback, term_logger]

    base_lr = 7.31e-5  # v4 best
    ...
    model = A2C(
        "CnnPolicy",
        train_env,
        n_steps=256,                 # ← from v4 best
        learning_rate=lr_schedule,   # ← decay with 10% floor
        gamma=0.999,                 # ← from v4 best
        gae_lambda=0.97,             # ← from v4 best
        ent_coef=0.00133,            # ← from v4 best (~1.33e-3)
        vf_coef=0.345,               # ← from v4 best
        max_grad_norm=1.0,           # ← from v4 best
        use_rms_prop=True,           # A2C default; stable for vision
        normalize_advantage=True,    # good with 2k+ batch
        policy_kwargs=dict(
            features_extractor_class=NatureCNN,
            features_extractor_kwargs=dict(features_dim=512, normalized_image=False),
            net_arch=dict(
                pi=[256, 256],       # ← from v4 best
                vf=[768, 768],       # ← from v4 best
            ),
        ),
        verbose=1,
    ...
    model.learn(
        total_timesteps=50_000_000,  # 10M timesteps total training
        callback=callbacks,
        tb_log_name="mario_A2C",
        progress_bar=True,
    )
    ...
    if os.path.exists(best_model_path):
        test_model = A2C.load(best_model_path)
    ...
    test_env = VecVideoRecorder(
        ...
        video_length=10000,
        name_prefix="mario_A2C_gameplay",
    )
    ...
    # Create DataFrame with episode results
    ...
    # Save to CSV
    results_csv_path = os.path.join(exp_dir, "test_results.csv")
