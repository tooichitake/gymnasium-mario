...
Train on 1-1 stage with Nature CNN architecture
"""
...
def objective(trial):
    ...
    # Keep categorical for power-of-2 values
    n_steps = trial.suggest_categorical("n_steps", [1024, 2048, 4096])
    batch_size = trial.suggest_categorical("batch_size", [64, 128, 256, 512, 1024, 2048, 4096])

    # Convert to continuous variables for better optimization with GPSampler
    ...
    pruning_callback = OptunaPruningCallback(
        ...
        eval_freq=10000 // 4,  # 10000 steps / n_envs = 2500 calls
        n_eval_episodes=1,
    )

    # Use Nature CNN architecture
    model = PPO(
        "CnnPolicy",
        train_env,
        n_steps=n_steps,
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="mario_ppo_hyperparameter_tuning_nature_cnn",
        direction="maximize",
        sampler=sampler,
    ...
    print("\n" + "=" * 80)
    print(f"Best trial: {study.best_trial.number} | Best reward: {study.best_value:.2f}")
    print(f"Study saved to database: {storage_name}")
