...
class NatureCNN(BaseFeaturesExtractor):
    """
    CNN architecture for image-based observations (Mnih et al. 2015).

    Architecture:
        - Conv2d(in→32, kernel=8, stride=4) → ReLU
        - Conv2d(32→64, kernel=4, stride=2) → ReLU
        - Conv2d(64→64, kernel=3, stride=1) → ReLU
        - Flatten → Linear(n_flatten→512)

    :param observation_space: The observation space of the environment
    :param features_dim: Number of features extracted (output of final linear layer, default 512)
    :param channels: List of output channels for 3 conv layers (default [32, 64, 64])
...
if __name__ == "__main__":
    ...
    train_env = make_mario_env(
        ...
        env_kwargs={"stages": ['1-1', '1-2', '1-3', '1-4', '2-1', '2-2', '2-3', '2-4']},
    ...
    eval_env = make_mario_env(
        ...
        seed=1,
        wrapper_kwargs={"frame_skip": 4, "screen_size": 84},
        use_vec_normalize=False,  # No normalization for evaluation - use raw rewards
    ...
    # IMPALA CNN: 1×=[16,32,32], 2×=[32,64,64], 4×=[64,128,128]
    # Nature CNN: 1×=[32,64,64], 2×=[64,128,128]
    model = PPO(
        "CnnPolicy",
        train_env,
        n_steps=2048,
        batch_size=4096,  # Increased for better gradient estimates
        n_epochs=20,  # Reduced to prevent overfitting on mini-batches
        learning_rate=3e-4,
        gamma=0.987,  # Screen-width planning horizon (~53 steps, 3.5 sec game time)
        gae_lambda=0.95,
        ent_coef=0.01,
        clip_range=0.2,
        vf_coef=0.5,
        ...
        policy_kwargs=dict(
            ...
            features_extractor_kwargs=dict(
                features_dim=256, channels=[16, 32, 32], normalized_image=False  # 2× base for 8-stage training
            ),
